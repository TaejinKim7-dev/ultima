...
User follow-up: owner accepted feature-branch + PR implementation flow as the normal/default approach. `main` stays stable; Todo branches merge by PR with RED/GREEN logs, Unit Test result, and QA evidence.
User follow-up: owner said to make everything public because the repository is not sensitive. Interpreted and recorded as public-by-default for plan/docs/implementation/vendor source/Korean translation source/workflows/Pages deployment, while still excluding original Ultima IV data, private extracted corpus, user saves, and secrets. Pages deploys automatically after PR merge to `main` once release checks pass.
